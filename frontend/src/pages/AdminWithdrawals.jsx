...
export default function AdminWithdrawals() {
  ...
  const withdrawals = useMemo(() => requests.filter((request) => request.type === 'withdraw'), [requests])
  const pending = useMemo(() => withdrawals.filter((request) => request.status === 'pending').length, [withdrawals])
  const approvedAmount = useMemo(
    () => withdrawals.filter((request) => request.status === 'approved').reduce((sum, request) => sum + Number(request.amount), 0),
    [withdrawals],
  )
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid sm:grid-cols-2 xl:grid-cols-4 gap-4">
        <StatTile icon={DollarSign} label="Total Withdrawals" value={String(withdrawals.length)} tone="from-indigo-500/25 to-indigo-400/10" />
        <StatTile icon={Clock3} label="Pending" value={String(pending)} tone="from-amber-500/25 to-amber-400/10" />
        <StatTile icon={AlertTriangle} label="Approved Value" value={`$${approvedAmount.toLocaleString()}`} tone="from-red-500/25 to-rose-400/10" />
        <AdminPanel className="p-4">
          <p className="text-xs uppercase tracking-wide text-gray-400 mb-2">7 Day Withdrawal Volume</p>
          <TinyBars values={dailyBars.length ? dailyBars : [1]} color="from-red-400 to-indigo-500" />
        </AdminPanel>
      </div>

      <AdminPanel className="p-5">
        <SectionTitle title="Withdrawal Requests" subtitle="Review and approve outgoing withdrawals" />
