...
export default function AdminUsers() {
  ...
  const handleManualBalance = async () => {
    ...
    try {
      if (balanceType === 'set') {
        await adminApi.setBalance(selectedUser.id, Number(balanceAmount))
        setActionMessage(`Balance set to: $${Number(balanceAmount).toLocaleString()}`)
      } else {
        const delta = balanceType === 'add' ? Number(balanceAmount) : -Number(balanceAmount)
        await adminApi.updateBalance(selectedUser.id, delta)
        setActionMessage(`Balance ${balanceType === 'add' ? 'added' : 'deducted'}: $${balanceAmount}`)
      }
      loadUsers()
    ...
    <div className="space-y-5">
                ...
                <tr key={user.id} className="border-b border-white/[0.06]">
                  ...
                  <td className="py-3 pr-3">
                    <div className="flex flex-wrap gap-2">
                      <button onClick={() => handleBalanceClick(user)} className="px-2.5 py-1.5 rounded-lg bg-cyan-500/20 text-cyan-300 border border-cyan-400/30 text-xs flex items-center gap-1"><Wallet className="w-3 h-3" /> Balance</button>
      ...
      {showBalanceModal && (
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="w-full max-w-md rounded-3xl border border-white/[0.12] bg-gradient-to-br from-[#171726] to-[#101019] p-5 sm:p-6">
            <div className="flex items-center justify-between gap-3 mb-4">
              <h3 className="text-xl font-semibold text-white">Adjust Balance: {selectedUser?.username}</h3>
              <button onClick={() => setShowBalanceModal(false)} className="w-9 h-9 rounded-lg bg-white/10 hover:bg-white/20 transition-colors flex items-center justify-center text-white">
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            <div className="space-y-4">
              <div className="p-4 rounded-xl bg-[#0d0d15] border border-white/[0.08]">
                ...
                <p className="text-2xl font-bold text-cyan-300">${Number(selectedUser?.balance || 0).toLocaleString()}</p>
              </div>

              <div>
                <label className="text-sm text-gray-300 mb-2 block">Operation</label>
                <div className="flex gap-2">
                  <button
                    onClick={() => setBalanceType('add')}
                    className={`flex-1 py-3 rounded-xl border text-center font-medium transition-all ${balanceType === 'add' ? 'bg-emerald-500/20 border-emerald-400/40 text-emerald-300' : 'border-white/10 text-gray-300 hover:bg-white/5'}`}
                  >
                    Add Balance
                  </button>
                  <button
                    onClick={() => setBalanceType('set')}
                    className={`flex-1 py-3 rounded-xl border text-center font-medium transition-all ${balanceType === 'set' ? 'bg-cyan-500/20 border-cyan-400/40 text-cyan-300' : 'border-white/10 text-gray-300 hover:bg-white/5'}`}
                  >
                    Set Exact
                  </button>
                  <button
                    onClick={() => setBalanceType('deduct')}
                    className={`flex-1 py-3 rounded-xl border text-center font-medium transition-all ${balanceType === 'deduct' ? 'bg-red-500/20 border-red-400/40 text-red-300' : 'border-white/10 text-gray-300 hover:bg-white/5'}`}
                  >
                    Deduct
                  </button>
                </div>
              ...
              <div>
                <label className="text-sm text-gray-300 mb-2 block">
                  {balanceType === 'set' ? 'New Balance Amount ($)' : 'Amount ($)'}
                </label>
                <input
                  type="number"
                  value={balanceAmount}
                  ...
                  className="w-full px-4 py-3 rounded-xl bg-[#0a0a0f] border border-white/[0.12] text-white focus:outline-none focus:border-cyan-500"
                />
              </div>

              {balanceType === 'set' && balanceAmount && (
                <div className="p-3 rounded-xl bg-cyan-500/10 border border-cyan-400/30">
                  <p className="text-sm text-cyan-300">
                    New balance will be set to: <span className="font-bold">${Number(balanceAmount).toLocaleString()}</span>
                  </p>
                </div>
              )}

              {balanceType === 'add' && balanceAmount && (
                <div className="p-3 rounded-xl bg-emerald-500/10 border border-emerald-400/30">
                  <p className="text-sm text-emerald-300">
                    Balance will increase by: <span className="font-bold">+${Number(balanceAmount).toLocaleString()}</span>
                  </p>
                </div>
              )}

              {balanceType === 'deduct' && balanceAmount && (
                <div className="p-3 rounded-xl bg-red-500/10 border border-red-400/30">
                  <p className="text-sm text-red-300">
                    Balance will decrease by: <span className="font-bold">-${Number(balanceAmount).toLocaleString()}</span>
                  </p>
                </div>
              )}
            </div>
            ...
            {actionMessage && (
              <div className={`mt-4 p-3 rounded-xl text-sm ${actionMessage.includes('added') || actionMessage.includes('deducted') || actionMessage.includes('set') ? 'bg-emerald-500/20 text-emerald-300 border border-emerald-400/30' : 'bg-red-500/20 text-red-300 border border-red-400/30'}`}>
                {actionMessage}
              </div>
            )}

            <div className="flex flex-wrap gap-3 mt-5">
              <button onClick={handleManualBalance} disabled={!balanceAmount || balanceLoading} className="flex-1 px-4 py-2.5 rounded-xl bg-gradient-to-r from-indigo-600 to-cyan-500 text-white font-semibold hover:from-indigo-500 hover:to-cyan-400 transition-all disabled:opacity-50 disabled:cursor-not-allowed">
                {balanceLoading ? 'Processing...' : 'Confirm'}
              </button>
              <button onClick={() => setShowBalanceModal(false)} className="flex-1 px-4 py-2.5 rounded-xl border border-white/20 text-gray-200 hover:bg-white/5 transition-colors">
                Cancel
              </button>
            </div>
          </div>
